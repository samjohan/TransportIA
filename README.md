...
3. Envía `/gasto` (o toca el botón "📝 Registrar gasto") para empezar a registrar uno — el bot pide primero la foto del recibo (usa el ícono de cámara o clip 📎 de Telegram para tomarla o adjuntarla; el bot no puede abrir la cámara por ti), o toca "Sin foto"; luego pregunta si quieres agregar una segunda foto (ej. el reverso) o toca "Listo, continuar". Lee ambas (QR primero si traen uno, si no OCR con Tesseract) y combina lo que encuentre en cada una; si detecta un monto lo ofrece como botón más adelante en vez de tener que escribirlo, y si detecta impuestos/factura/NIT los adjunta directo al gasto. Después pregunta ruta (solo rutas `pendiente`/`en_curso`), categoría, monto (sugerido o escrito) y nota opcional, y confirma al terminar — mencionando lo detectado. `/cancelar` aborta el flujo en cualquier momento.
